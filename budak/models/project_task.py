...
class ProjectTask(models.Model):
    ...
    selection_driveable = fields.Selection(
        selection=[('fahrbereit', 'fahrbereit'), ('nicht fahrbereit', 'nicht fahrbereit'),
                   ('bedingt fahrfähig bis zur Werkstatt', 'bedingt fahrfähig bis zur Werkstatt')])
    selection_driving_safety = fields.Selection(
        selection=[('verkehrsicher', 'verkehrsicher'), ('nicht verkehrsicher', 'nicht verkehrsicher')])
    ...
    selection_average = fields.Selection(
        selection=[('durchschnittlich', 'durchschnittlich'), ('überdurchschnittlich', 'überdurchschnittlich')])
    ...
    selection_car_average = fields.Selection(
        selection=[('überdurchschnittlich', 'überdurchschnittlich'), ('durchschnittlich', 'durchschnittlich')])
    ...
    selection_car_usable_dirty = fields.Selection(
        selection=[('Fahrzeug einsatzbereit verschmutzt', 'Fahrzeug einsatzbereit verschmutzt')])
    ...
    def _get_document_folder(self):
        ...
        folder_year_id = self.env['documents.folder'].search([('name', '=', current_year)])
        ...
        if not folder_month_id:
            folder_month_id = self.env['documents.folder'].create({
                'name': current_month,
                'parent_folder_id': folder_year_id.id})
    ...
    #     print('self.attachment_ids',attachment_ids)
    #     if self.name and self.stage_id and self.stage_id.name != "New" and not attachment_ids:
